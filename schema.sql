...
CREATE TABLE `tb_order` (
  ...
  `receiver_address` VARCHAR(255) NOT NULL COMMENT '收货地址或自提点说明',
  `delivery_type` TINYINT(1) DEFAULT 1 COMMENT '取货方式：1-线上配送（平台统一配送），2-线下自提（唯一门店）',
  ...
  `refund_prev_status` TINYINT(1) DEFAULT NULL COMMENT '申请退款前状态（1/2/3）',
  `refund_reason` VARCHAR(500) DEFAULT NULL COMMENT '用户退款/退货说明',
  `refund_apply_time` DATETIME DEFAULT NULL COMMENT '退款申请时间',
  `refund_audit_time` DATETIME DEFAULT NULL COMMENT '退款审核时间',
  `refund_admin_remark` VARCHAR(500) DEFAULT NULL COMMENT '驳回原因等',
  `create_time` DATETIME DEFAULT NULL COMMENT '创建时间',
  `update_time` DATETIME DEFAULT NULL COMMENT '更新时间',
  `deleted` INT(11) DEFAULT 0 COMMENT '是否删除：0-未删除，1-已删除',
  PRIMARY KEY (`id`),
...
-- 若订单表仍含历史字段 store_id，单门店升级可执行：见 migrations/drop_order_store_id.sql
...
CREATE TABLE `tb_culture_content` (
  ...
  `type` TINYINT(1) DEFAULT 1 COMMENT '分类类型：1-白茶知识，2-制作工艺，3-品鉴技巧，4-历史文化，5-培训专区',
...
CREATE TABLE `tb_experience_activity` (
  ...
  `pick_meeting_point` VARCHAR(300) DEFAULT NULL COMMENT '采摘招募：集合地点（仅审核通过用户可见）',
  `pick_contact_line` VARCHAR(200) DEFAULT NULL COMMENT '采摘招募：联系人/电话',
  `pick_notice` TEXT DEFAULT NULL COMMENT '采摘招募：上岗须知',
  `training_materials` TEXT DEFAULT NULL COMMENT '批发培训：学习资料/链接',
  `training_extra_hint` VARCHAR(500) DEFAULT NULL COMMENT '批发培训：费用与参训补充说明',
  `create_time` DATETIME DEFAULT NULL COMMENT '创建时间',
  `update_time` DATETIME DEFAULT NULL COMMENT '更新时间',
  `deleted` INT(11) DEFAULT 0 COMMENT '是否删除：0-未删除，1-已删除',
  PRIMARY KEY (`id`),
...
CREATE TABLE `tb_quiz_question` (
  ...
  `category` TINYINT(1) DEFAULT 1 COMMENT '分类：1-互动，2-文化，3-活动，4-培训专区',
...
('福鼎白茶平台线下自提中心', '平台唯一线下自提点；门店配货完成后，请凭订单到店自提。', NULL, '福建省', '宁德市', '福鼎市', '桐城街道玉龙北路128号', 120.2178000, 27.3255000, '0593-1234567', '09:00-21:00', 1, NOW(), NOW());
...
CREATE TABLE `tb_industry_application` (
  ...
  `join_code` VARCHAR(64) DEFAULT NULL COMMENT '审核通过后对接码',
  `checked_in_at` DATETIME DEFAULT NULL COMMENT '采摘到岗签到时间',
  `create_time` DATETIME DEFAULT NULL COMMENT '创建时间',
  `update_time` DATETIME DEFAULT NULL COMMENT '更新时间',
  `deleted` INT(11) DEFAULT 0 COMMENT '是否删除：0-未删除，1-已删除',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_activity` (`user_id`, `activity_id`),
  UNIQUE KEY `uk_industry_join_code` (`join_code`),
  KEY `idx_activity_id` (`activity_id`),
  KEY `idx_user_id` (`user_id`),
  KEY `idx_status` (`status`),
  KEY `idx_create_time` (`create_time`)
